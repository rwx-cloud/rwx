...
# shallowness can't be what fails the push (that path is already covered by
...
if [ "$(git rev-parse --is-shallow-repository)" = "true" ]; then
  ...
  # credential.helper resets the helper list so the fetch goes out anonymously.
...
# sandbox workspace really is a depth-1 clone. A fix that deepens the sandbox
# only when the push needs missing history leaves this assertion true; a fix
# that deepens unconditionally will need it relaxed.
...
# Using the current tree keeps the working directory untouched (this script
# lives in it), and the second parent is an ancestor of HEAD, so it exists
# locally but was pruned from the sandbox's depth-1 clone.
