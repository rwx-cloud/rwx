...
"${RWX_CLI}" sandbox exec --id "$SANDBOX_RUN_ID" -- sh -c 'echo "new sandbox file" > new-sandbox-file.txt'
...
"${RWX_CLI}" sandbox exec --id "$SANDBOX_RUN_ID" -- sh -c 'echo "# Sandbox modification" >> go.mod'
...
sandbox_modified_sha=$("${RWX_CLI}" sandbox exec --id "$SANDBOX_RUN_ID" -- sha1sum go.mod | awk 'NR==1{print $1}')
