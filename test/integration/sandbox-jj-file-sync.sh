...
# Colocate the existing git checkout so jj drives the same working copy. Git is
# preferred for colocated repos, so RWX_VCS forces the jj backend under test.
#
# Colocating leaves the imported bookmark on @'s parent, so the working copy is
# an anonymous change: the CLI reports no branch and keys the sandbox session by
# the working copy's change id. That key has to survive the edits below, since
# jj rewrites @ on every snapshot.
