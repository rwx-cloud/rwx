...
// configuration. jj is pointed at an empty config file rather than /dev/null so
// the tests do not depend on how it handles a character device.
...
func TestGetBranch(t *testing.T) {
	...
	// The clone's `main` bookmark stays on the base commit as @ moves ahead of
	// it. Reporting a bookmark the working copy has already left behind would
	// file runs and sandbox sessions under a branch this change is not on.
	t.Run("is empty when the only bookmark is on an ancestor", func(t *testing.T) {
...
func TestGetHead(t *testing.T) {
	eachLayout(t, "clone", func(t *testing.T, f fixture) {
		...
		require.Equal(t, head, f.client.GetHead())
...
// GetShortHead keys sandbox sessions whenever GetBranch is empty, which under jj
// is the common case. A commit-id key would change on every edit and strand a
// sandbox each time.
...
func TestGetCommit(t *testing.T) {
	...
	t.Run("falls back to the working copy in a repository with no remotes", func(t *testing.T) {
		eachLayout(t, "init-uncommitted", func(t *testing.T, f fixture) {
			sha, err := f.client.GetCommit()
			require.NoError(t, err)
			require.Equal(t, f.client.GetHead(), sha)
		})
	})

	// An anonymous change is jj's detached HEAD. The git backend answers with
	// HEAD there rather than erroring so the caller can still run and patch on
	// top; these two cases keep the backends in step. Never the virtual root.
	...
	t.Run("falls back to the working copy when no remote is named origin", func(t *testing.T) {
		eachLayout(t, "clone-remote-named-upstream", func(t *testing.T, f fixture) {
			sha, err := f.client.GetCommit()
			require.NoError(t, err)
			require.Equal(t, f.client.GetHead(), sha)
	...
	// A bookmark on @ is a named position, so the misconfiguration is reported
	// rather than papered over — matching git's behavior on a branch.
	...
	t.Run("errors when a bookmark names the change and no remote is origin", func(t *testing.T) {
		eachLayout(t, "clone-remote-named-upstream-bookmark", func(t *testing.T, f fixture) {
...
// staleHead reads @ without letting jj snapshot, so it observes the working copy
// commit rather than moving it.
...
// it must not rewrite @. The base is the same either way: snapshotting replaces
// @ in place without touching its ancestors.
func TestGetCommitDoesNotSnapshotTheWorkingCopy(t *testing.T) {
	eachLayout(t, "clone-local-work", func(t *testing.T, f fixture) {
		...
		// The snapshot still happens where it is actually needed, and the base is
		// unchanged by it — which is what makes the stale read above safe.
		head, err := f.client.GetHeadCommit()
		require.NoError(t, err)
...
// Uncommitted work reaches the sandbox inside the pushed commit rather than as a
// dirty patch applied on top of it, so there is never dirty state to report.
...
// A secondary workspace has no .jj/repo of its own, only a file pointing at the
// primary one, so reaching the git store means following that indirection.
...
func TestPushRef(t *testing.T) {
	...
	// The refspec's commit has to already be in the backing git store, which only
	// happens once jj has snapshotted the working copy. Pushing a sha that was
	// never snapshotted has to say so rather than surfacing git's own error.
	t.Run("rejects a commit missing from the store", func(t *testing.T) {
		eachLayout(t, "clone", func(t *testing.T, f fixture) {
			...
			require.ErrorContains(t, err, "has not been snapshotted into the git store")
			require.ErrorContains(t, err, missing)
		})
	})

	// jj exports a conflicted change to git with one side of the conflict as its
	// content, so pushing it would send the sandbox files that are in neither the
	// developer's working copy nor either parent.
...
func TestApplyPatch(t *testing.T) {
	...
	// A non-colocated workspace has no .git of its own, so git apply only honors
	// .gitattributes when it is pointed at the backing store.
...
// jj silently skips files over snapshot.max-new-file-size, so they vanish from
// every patch. The warning it prints on the way past has to reach the user.
