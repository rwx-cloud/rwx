...
func TestAPIClient_UploadPackage(t *testing.T) {
	...
	t.Run("surfaces a multiline validation error verbatim", func(t *testing.T) {
		c := api.NewClientWithRoundTrip(func(req *http.Request) (*http.Response, error) {
			return &http.Response{
				Status:     "400 Bad Request",
				StatusCode: 400,
				Body:       io.NopCloser(strings.NewReader(`{"error":"Upload did not contain rwx-package.yml\n\nFiles in upload:\n* README.md"}`)),
			}, nil
		})
		...
		require.EqualError(t, err, "Upload did not contain rwx-package.yml\n\nFiles in upload:\n* README.md")
	})
