...
func TestService_BuildPackagePreservesVisibility(t *testing.T) {
	for _, filename := range []string{"rwx-package.yml", "mint-leaf.yml"} {
		for _, manifest := range []string{
			...
			t.Run(filename+"/"+manifest, func(t *testing.T) {
				s := setupTest(t)
				dir := t.TempDir()
				...
				s.mockAPI.MockUploadPackage = func(cfg api.UploadPackageConfig) (*api.UploadPackageResult, error) {
					archive, err := io.ReadAll(cfg.Contents)
					require.NoError(t, err)
					reader, err := readZip(t, archive)[filename].Open()
					require.NoError(t, err)
					defer reader.Close()
