...
func TestService_ExecSandbox_Sync(t *testing.T) {
	...
	t.Run("creates sync ref even when no local changes", func(t *testing.T) {
		...
		for i, cmd := range commandOrder {
			if strings.Contains(cmd, "update-ref refs/rwx-sync HEAD") && strings.Contains(cmd, "commit --allow-empty") {
				snapshotIdx = i
				break
			}
		}
	...
	t.Run("syncs changes and reverts sandbox after pull", func(t *testing.T) {
		...
		for i, cmd := range commandOrder {
			...
			if strings.Contains(cmd, "git reset --mixed HEAD~1") {
				resetIdx = i
			}
		}
	...
	t.Run("recovers pending untracked sandbox files before resetting to local head", func(t *testing.T) {
		...
		runID := "run-pending-untracked"
		address := "192.168.1.1:22"
		localHead := "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"
		pendingPatch := "diff --git a/created.txt b/created.txt\nnew file mode 100644\nindex 0000000..257cc56\n--- /dev/null\n+++ b/created.txt\n@@ -0,0 +1 @@\n+created\n"
		...
		setup.mockGit.MockApplyPatch = func(patch []byte) *exec.Cmd {
			order = append(order, "local-apply")
			appliedPatch = string(patch)
		...
		setup.mockSSH.MockExecuteCommand = func(cmd string) (int, error) {
			switch {
			case strings.Contains(cmd, "rev-parse --verify -q refs/rwx-sync"):
				order = append(order, "sync-ref-check")
				return 0, nil
			case strings.Contains(cmd, "cat-file -e"):
				return 0, nil
		...
		setup.mockSSH.MockExecuteCommandWithOutput = func(cmd string) (int, string, error) {
			...
			case strings.Contains(cmd, "git ls-files --others"):
				lsCalls++
				if lsCalls == 1 {
					return 0, "created.txt\n", nil
				}
				return 0, "", nil
			case strings.Contains(cmd, "git add -N"):
				order = append(order, "intent-to-add")
				return 0, "", nil
			case strings.Contains(cmd, "git diff refs/rwx-sync"):
				diffCalls++
				if diffCalls == 1 {
					order = append(order, "pending-diff")
					return 0, pendingPatch, nil
				}
				return 0, "", nil
		...
		require.NoError(t, err)
		require.Equal(t, runID, result.RunID)
		require.Equal(t, pendingPatch, appliedPatch)
		require.GreaterOrEqual(t, diffCalls, 2)
		pendingDiffIndex := slices.Index(order, "pending-diff")
		checkoutIndex := slices.Index(order, "checkout")
		require.NotEqual(t, -1, pendingDiffIndex)
		require.NotEqual(t, -1, checkoutIndex)
		require.Less(t, pendingDiffIndex, checkoutIndex)
	})
	...
	t.Run("stops sandbox and removes session when .git directory is missing", func(t *testing.T) {
		...
		setup.mockSSH.MockExecuteCommand = func(cmd string) (int, error) {
			if strings.Contains(cmd, "test -d .git") {
...
func TestService_ExecSandbox_PullPatchFailureRecovery(t *testing.T) {
	t.Run("retries with --reject on apply failure and reports .rej files", func(t *testing.T) {
		...
		require.Error(t, err)
		require.Nil(t, result)
		require.Contains(t, err.Error(), "file.txt (see file.txt.rej)")
		require.Contains(t, err.Error(), "Resolve the conflicts")
	})
	...
	t.Run("saves rejected patch to .rwx/sandboxes", func(t *testing.T) {
		...
		require.Contains(t, err.Error(), "patch-rejected.diff")
	})
...
func TestService_ExecSandbox_Lock(t *testing.T) {
	...
	t.Run("pre-exec cleanup runs between lock and sync", func(t *testing.T) {
		...
		// Verify the pre-exec checkout appears after lock but before the user command.
		lockIdx := -1
		checkoutStartIdx := -1
		checkoutIdx := -1
		checkoutEndIdx := -1
		...
		for i, cmd := range commandOrder {
			...
			if strings.Contains(cmd, "git checkout -f") && checkoutIdx == -1 {
				checkoutIdx = i
			}
		}
		require.NotEqual(t, -1, checkoutIdx, "pre-exec checkout should run")

		for i := checkoutIdx - 1; i >= 0; i-- {
			if commandOrder[i] == "__rwx_sandbox_sync_start__" {
				checkoutStartIdx = i
				break
			}
			if commandOrder[i] == "__rwx_sandbox_sync_end__" {
				break
			}
		}
		for i := checkoutIdx + 1; i < len(commandOrder); i++ {
			if commandOrder[i] == "__rwx_sandbox_sync_end__" {
				checkoutEndIdx = i
				break
			}
		...
		require.Greater(t, checkoutStartIdx, lockIdx, "checkout sync_start should follow lock")
